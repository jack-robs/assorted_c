...
1. n^2. Specifically, I think the actual size requires are (n^2 * 4 bytes), if the integer 1/0 are 4 bytes.
2. No I don't think so. Even if a node has no in/out degrees, that would still be represented as a row of 0's in that node's row. 
...
1. No, the sort has to start with a vertex with a in-degree of 0, in orderto start placing vertices "after" it. Undirected graphs don't indicate whether a degree is in or out. Similiar, topo sorts by D(u,v) edge u has to come before v. If the graph was cyclic, that decision couldn't be made, or I think 1 direction (u to v) would be shown, but likely v to u wouldn't be. A cycle could be detected by seeing if each node had a path back to itself, and if you received a T/F return per node, you could determien if the node has a cycle
